...
【厳守・出力形式】
- **応答は上記の JSON オブジェクト1つのみ** を返してください
- JSON の前後に説明文・挨拶・コードブロック記号(``` 等)を**一切含めない**
- いきなり `{{` から始めて `}}` で閉じる
- 全フィールドを必ず埋める（空文字 "" 不可、最低でも妥当な短文で埋める）
"""


def call_claude_step1(palm_json: dict, hand: str) -> dict | None:
    """Step 1: Claude で手相だけの一次解釈を生成。
    プロンプト拡張に追随して max_tokens 4000、temperature 0.2 で安定 JSON を取る。
    Opus 4.7 は assistant prefill 非対応のため、prompt 末尾の JSON-only 指示で抑え込む。
    """
