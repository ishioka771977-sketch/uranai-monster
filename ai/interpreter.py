...
### 分量（絶対ルール）
- reading本文は**1,800文字を目安**に書く（1,600文字未満は鑑定失敗とみなす。上限2,200文字）
- 短すぎる鑑定は「浅い」と感じる。たっぷり語ること
- ただし冗長な繰り返しで水増ししない。段落を一つ増やす時は新しい視点・素材を持ち込む
- ※2026-07-13 下限を引き上げ（4エンジン読み比べで、指示に忠実なエンジンほど
  下限側に寄り「薄い」と評価された実測に基づく。ひでさん指示）
"""
...
{{
  ...
  "reading": "本文の鑑定文（v3）。**自然に必要な長さで書く**。長さ自体は目標ではなく、「読み手が最後まで読みたくなるか」だけが判断基準。タイプ分類・教科書解説は削るが、必要な情景・専門用語の優しい解説は残す。ただし1,800文字を目安とし、1,600文字を下回らない（2026-07-13 読み比べ実測に基づく・ひでさん指示）。",
...
{{
  ...
  "reading": "鑑定文（v3・自然に必要な長さで書く）。長さ自体は目標ではなく「読み手が最後まで読みたくなるか」が判断基準。ただし1,800文字を目安とし、1,600文字を下回らない（2026-07-13 読み比べ実測に基づく・ひでさん指示）。",
  "closing": "締めの一言（30〜60文字）"
}}
...
{{
  ...
  "reading": "鑑定文（v3）。**自然に必要な長さで書く**。長さ自体は目標ではなく、「読み手が最後まで読みたくなるか」だけが判断基準。タイプ分類・教科書解説は削るが、必要な情景・専門用語の優しい解説は残す。ただし1,800文字を目安とし、1,600文字を下回らない（2026-07-13 読み比べ実測に基づく・ひでさん指示）。",
  "closing": "締めの一言（30〜60文字）"
}}
...
{{
  ...
  "reading": "本文の鑑定文（v3）。以下の構造で書く。**自然に必要な長さで書く**。長さ自体は目標ではなく、「読み手が最後まで読みたくなるか」だけが判断基準。タイプ分類・教科書解説は削るが、必要な情景・専門用語の優しい解説は残す。ただし1,800文字を目安とし、1,600文字を下回らない（2026-07-13 読み比べ実測に基づく・ひでさん指示）。",
  "closing": "締めの一言（30〜60文字）"
}}
...
def _v3_critical_prefix(bundle: "DivinationBundle" = None) -> str:
    ...
    if bundle is not None:
        try:
            ...
            future_10_year = current_year + 10
            future_10_age = age + 10
            future_20_year = current_year + 20
            future_20_age = age + 20
            age_block = (
                f"\n## 【★★★ 鑑定対象者の年齢（絶対遵守）★★★】\n"
                f"**この人は {current_year}年現在、満 {age} 歳です。**\n"
                f"**10年後（{future_10_year}年）は満 {future_10_age} 歳になります。**\n"
                f"**20年後（{future_20_year}年）は満 {future_20_age} 歳になります。**\n"
                f"鑑定文中で年齢を語るなら、上記の値以外を書いてはいけない。\n"
                f"大運・大限・流年などで出てくる「○○〜○○歳」の数字は、\n"
                f"単なる干支の切り替わり年齢範囲を示す **理論上のレンジ表示** であって、\n"
                f"この人の現在年齢でも、将来年齢でもない。レンジの数字を年齢と読み違えるのは鑑定失敗扱い。\n"
                f"未来年齢を語る場合は必ず本ブロックで明示した値（10年後＝{future_10_age}歳、\n"
                f"20年後＝{future_20_age}歳）を使うこと。Python側で正確に計算済み。\n\n"
            )
        except Exception:
            pass
    return age_block + """
...
### 専門用語の優しい解説ルール（v3 §4 新要素・★★★必須項目★★★）

**★★★ 鑑定文中で使う専門用語には、最低1回は「優しい解説」を必ず添えること。これが無い鑑定文は v3 失敗扱いとなる。★★★**

読み手は土建屋のおじさん・OLさん・初めて占いに触れる人。「中心星」「牽牛星」「天中殺」「身宮」「殺破狼」「離宮回座」「ライフパス」「ASC」「アスペクト」「用神」等の用語をそのまま投げると読み手は脱落する。**1度目に登場した時に、必ずカッコか短いフレーズで意味を添える。**
...
| ○ カッコ解説 | 「7ハウス（結婚やパートナーシップを映す場所）に土星（時間と忍耐の星）が座っている」 |
| ◎ 情景に溶かす | 「結婚を映す場所に、重い星が座る。パートナーを選ぶまでに時間がかかる人。でも選んだら、深い」 |

#### 3つの絶対ルール

1. 専門用語が初めて登場する時、**必ず**カッコか短いフレーズでイメージを添える
2. 情景の中に溶かせるならそちらが上（◎）
3. 同じ用語の解説は1鑑定で1回だけ（2回目以降は解説不要）

#### 主要専門用語の解説テンプレート（このまま使ってよい）

- **算命学**: 中心星（人格の核にある星）/ 牽牛星（名誉とプライドで動く星）/ 天中殺（宿命が一時停止する2年間）/ 日干（あなた自身を表す一文字）/ 三巳格（火性が三つ揃った特殊配置）
- **四柱推命**: 用神（あなたの命式に最も必要な処方箋）/ 身弱（外の力を借りる設計）/ 大運（10年ごとに切り替わる人生の大きな潮流）/ 通変星（自分と他者の関係を表す星）
- **紫微斗数**: 命宮（あなたの魂の住所）/ 身宮（35歳以降に表に出てくるもう一つの自分）/ 殺破狼（変動と開拓の宿命を持つ配置）/ 化忌（人生の痛点）/ 流年命宮（その年だけ主役になる宮）
- **九星気学**: 本命星（生まれ年で決まる基本エネルギー）/ 離宮回座（評価と露出の年）/ 中宮回座（八方塞がりに見えるが内向きの年）/ 祐気取り（吉方位への旅行で運気を補充する開運法）
- **数秘術**: ライフパスナンバー（生年月日から導かれる人生のテーマ番号）/ パーソナルイヤー（9年周期の今の位置）/ 誕生日数（あなたの個性を表す数字）
- **万象学**: エネルギー指数（気力・体力・行動力の総量を数値化したもの）/ 五本能（攻撃・守備・表現・魅力・学習の5分類）
- **西洋占星術**: トランジット（今の天体配置があなたに与える影響）/ アスペクト（天体同士が作る角度）/ ASC（社会に見せている第一印象の星座）/ MC（社会的到達点を示す）/ ハウス（人生の各領域）/ オーブ（角度のズレ）

→ 上記テンプレートはそのまま使ってよい。ただし「情景に溶かす」のがベストなので、可能なら独自の比喩で言い換える。

#### 違反例（これらは全部 v3 失敗）

- 「あなたの中心星は牽牛星です」← 解説なし、いきなり用語投げ
- 「殺破狼の影響で破軍命の人は…」← 解説なし
- 「離宮回座の年だから露出が増える」← 「離宮回座」が何か説明なし
...
def _call_api_length_floor(prompt: str, max_tokens: int = 4000,
                            min_chars: int = 1600, target: str = "1,800") -> dict:
    """コース鑑定用: readingが下限字数未満なら1回だけ書き直しリトライ。

    2026-07-13 4エンジン読み比べの実測(短い出力ほど「薄い」と評価される・
    プロンプト指定だけでは字数が安定しない)に基づく下限保証。ひでさん指示。
    """
    result = _call_api(prompt, max_tokens=max_tokens)
    reading = result.get("reading") or ""
    if len(reading) < min_chars:
        retry_prompt = prompt + (
            f"\n\n【再指示】直前の出力のreadingは{len(reading)}文字で短すぎた。"
            f"内容の密度と文体を保ったまま、readingを{target}文字前後まで厚くして"
            "全体を書き直すこと。新しい段落には、まだ触れていない星・数字・具体的な"
            "情景など新しい素材を使う。水増しの繰り返しは禁止。"
        )
        try:
            result2 = _call_api(retry_prompt, max_tokens=max_tokens)
            if len(result2.get("reading") or "") > len(reading):
                print(f"[interpreter] length-floor retry: {len(reading)} -> "
                      f"{len(result2.get('reading') or '')} chars", flush=True)
                return result2
        except Exception as e:
            print(f"[interpreter] length-floor retry failed: {e}", flush=True)
    return result
...
def generate_sanmei_reading(bundle: DivinationBundle) -> dict:
    ...
    try:
        ...
        return _call_api_length_floor(_with_person(prompt, bundle, v3_polish=True), max_tokens=4000)
    except Exception as e:
...
def _format_seven_schools_summary(bundle: DivinationBundle) -> str:
    ...
    # 開運習慣L3〜L5用の追加データ(2026-07-15 Phase2拡張)
    if k.lucky_direction:
        lines.append(f"- 九星気学(方位): 今年の吉方位={k.lucky_direction}、今年のテーマ={k.year_theme}")
    if s.tenchusatsu_years:
        lines.append(f"- 天中殺の年: {', '.join(str(y) for y in s.tenchusatsu_years)}")
    if sh and sh.taiun_list:
        try:
            from datetime import date as _d
            age = (_d.today() - bundle.person.birth_date).days // 365
            cur = None
            for t in sh.taiun_list:
                if t.start_age_mansai <= age:
                    cur = t
                else:
                    break
            if cur:
                end = cur.start_age_mansai + 9
                lines.append(f"- 現在の大運(10年潮流): {cur.kanshi}({cur.start_age_mansai}〜{end}歳)、通変星={cur.tsuhensei}、十二運={cur.juni_unsei}")
        except Exception:
            pass
    kj = getattr(bundle, "kojindo", None)
    if kj:
        lines.append(f"- 古神道: 守護神={kj.god_name}(総本社: {kj.god_shrine})、人生フェーズ={kj.phase_name}(満{kj.current_age}歳)→次は{kj.next_phase_name}")
    return "\n".join(lines)
...
# ============================================================
KOJINDO_V2_PROMPT = """あなたは「占いモンスターくろたん」の古神道占いコースの鑑定文を書く。

古神道占いは7流派の「上」に立つ最上位コース。占術名を一切消し、その人の
行動・思考・身体感覚だけで語り、古事記・日本書紀・宮下文書・竹内文書・
風土記の神々の物語に「重ねる」鑑定。
...
**指針2：1柱の守護神に全部重ねるな。動的マッチングしろ**
- 7流派で被る「際立つ特徴」に合う物語をストックから選ぶ
- ただし鑑定文に使うのは**最大3本**。5本は多すぎる（神話紹介になる）

**指針3：物語は羅列ではなく一本の流れで溶かせ**
- ×「一つ目は○○の物語。二つ目は△△の物語」
- ○ 火→毒龍→水→蘇生、という流れの中に3つの物語が溶け込む

---

## この人のデータ（裏側・本文には占術名出さず体感だけ使え）
...
- 満{current_age}歳。フェーズ説明: 「{phase_line}」
- 次に来るフェーズ: 「{next_phase_line}」

### あなたに重なる物語の候補（マッチ済み {n_stories} 本）

{stories_block}

→ この中から **主物語1本 ＋ 副物語1〜2本 ＝ 最大3本**を選べ。
   `selected_story_ids` に採用した story_id（最大3本）を記録すること。
   4本以上は禁止。番号で並べず、一本の流れに溶かすこと。

### 推奨参拝候補（採用した物語に紐づくもの・押し付けない）
...
- 占術名・専門用語は一切使うな
- 読み手が「当たっている」と感じる密度を最優先。ここで信頼を獲得する
- 所作・数量・時間で書く（例「鎧が3枚ある」「朝5時に目が開く日は行き先のない移動を90分」「水辺に行くと呼吸が楽になる」）
- 第1幕の最後に、続きへ橋を架ける1行（例「——ここまで読んで『当たっている』と思ったなら、続きを読んでほしい。」）

### ■ 第2幕：あなたに重なる物語（全体の50%）
- 主物語1本 ＋ 副物語1〜2本 ＝ 最大3本
- **絶対に「一つ目は」「二つ目は」と番号で並べるな**
- 物語同士を一本の流れでつなげろ（例：火→毒龍→水→蘇生）
- 各物語の出典を自然に組み込め（正史=「神話に〜とある」/ 古史古伝=「ある伝承では〜と語られる」/ 風土記=「〇〇国風土記に」/ 神社縁起=「〇〇神社の縁起に」）
- 神名には読みと短い優しい解説をカッコで添える（初出時）
- 物語は「紹介」ではなく「この人の人生に重ねる」形で語れ
- 物語の転換点を、この人の人生の具体的事実（年齢・出来事）に結びつけろ

### ■ 第3幕：今のあなたへ（全体の20%）
- 「満{current_age}歳の今、この物語のどこにいるか」を1〜2行で
- 推薦神社を1つ。鑑定文の物語と直結する理由を添えろ（「もし足が向くなら」型の温度感）
...
- 占術用語: 流派名（算命学・四柱推命・西洋占星術・九星気学・数秘術・紫微斗数・万象学）/ 星名（牽牛星・車騎星・貫索星・鳳閣星・調舒星・禄存星・龍高星・玉堂星等）/ 「天中殺」「空亡」「用神」「身強」「身弱」「日干」/ 星座名（双子座・獅子座等）「太陽星座」「月星座」「ASC」「MC」「ハウス」「アスペクト」/ 九星名（本命星・五黄土星等）/ 「ライフパス」「個人年」/ 「命宮」「身宮」「殺破狼」「化忌」/ 「エネルギー指数」「五本能」/ 干支・五行（丙・辛・壬水・巳・申酉・丙午等）
- 物語の番号付き羅列（「一つ目は○○の物語」「二つ目は△△の物語」）
- 4本以上の物語を並べる
...
- **藤原基央メソッド**: 感情語禁止。所作と身体感覚で感情を代弁する
- **占いモンスターの4原理**:
...
## 参考鑑定文（くろたん手書き・品質の基準。このレベルを目指せ）

```
鎧が3枚ある。
名前で呼ばれるより肩書きで呼ばれるほうが楽で、
席を立つ時は誰よりも先に立ち、座る時は最後に座る。
止まると壊れるエンジンを積んでいて、
朝5時に目が開く日は、行き先のない移動を90分しないと収まらない。
水辺に行くと呼吸が楽になる。サウナの水風呂に沈んだ30秒が、
1週間を支えている。

——ここまで読んで「当たっている」と思ったなら、続きを読んでほしい。

あなたの人生は、瓊瓊杵尊（ににぎのみこと）の物語から始まる。
三種の神器を抱えて高天原から地上に降りた天孫。
ある伝承では、新婚の夜の畳をそのままに葦の船底に膝をつき、
妻は別働隊を率いて自ら将として四国へ向かったと語られる。
家庭より現場を選んだ夜が、あなたにもあったはず。
あの夜の畳の冷たさを、ニニギも知っている。

だが、ニニギの物語だけでは足りない。

あなたの内側には、ニニギの覚悟とは別の火がある。
火が4つ。宝石に火入れをする構造。
磨かれるためには焼かれなければならない。
だがその火が制御を超えた時、芦ノ湖を荒らした毒龍が現れる。
九頭龍——かつて人を苦しめた龍が、
万巻上人の法力で調伏されて龍神に変わった物語。
あなたの規格外のエネルギーも、喰えば龍神になる。
喰えなければ毒龍のまま。
33歳で倒産寸前の会社を背負ったのは、
毒龍を喰う器を作る運命の絶妙なプレゼントだった。

そして火を鎮めるのは、水。
少彦名命（すくなびこなのみこと）は大国主と共に国を造った小さな神。
ある日突然、常世に消えた。残された大国主は独りで立った。
だが伊予の伝承では、病に倒れた少彦名を
大国主が海の底から湯を引いて温め、蘇らせたとも語られる。
あなたの命式が「水を取れ」と言っているのは、
失ったものの中にしかない何かを、取りに行けということ。

満48歳。瓊瓊杵尊の物語で言えば、
建てた城の窓から次の主が見え始めている頃。
譲ることは負けではない。
譲れる者がいること自体が、築いた証拠。

もし足が向くなら、箱根の九頭龍神社。
芦ノ湖のほとり。毒龍が龍神に変わった場所。
あなたの火と水が、同じ湖面に映る場所。

名は遅れて届く。だが必ず届く。
```

※注: 上記サンプルの「火が4つ」「命式が」は説明のための例示。
実際の鑑定文では占術用語を出さず、体感パターンに変換して書くこと。
...
{{
  "headline": "鑑定全体を象徴する一行（15〜30文字・特定の神に偏らない）",
  "reading": "本文（3幕構成・占術名禁止・最大3本の物語を一本の流れに溶かす・番号付き羅列禁止）",
  "selected_story_ids": ["採用した story_id を最大3本"],
...
def _format_trait_sensations_block(sensations) -> str:
    """体感説明ブロックを整形（タグ→体感説明文）

    プロンプト内では占術名は出さない。タグはAI側のメモとして {} に入れる。
    """
    if not sensations:
        return "（体感パターン抽出なし）"
    lines = []
    for tag, sense in sensations:
        # タグは小さく、体感説明をメインに
        lines.append(f"- {sense}  _(内部タグ: {tag})_")
    return "\n".join(lines)


def _next_phase_line_lookup(next_phase_name: str) -> str:
    """次フェーズの説明文を体感マップから引く"""
    NEXT_PHASE_LINES = {
        "母の庇護": "庭に守られた根を持つ時期",
        "反抗と試練": "火傷から立ち上がる季節",
...
**指針4：占的が主役。神話は道具。「あなたのここに当たる」を毎所作で接続せよ**（★最重要★）
- 主役は **読み手の人生**。神話の場面は **照らす照明** にすぎない。
- ×「神話の風景を3段落書いてから最後に『これがあなたです』」（接続が遠い・物語が自走している）
- ○「神話の場面を2〜4行 → 即『あなたの◯◯がここと同じ』を所作で1〜2行 → 次の場面へ」（接続を毎回挟む）
- 神話の純粋な解説で1段落を消費するな。**1場面描いたら必ず「あなたの所作・数量・身体感覚」と接続する行を続ける**。
- 入力データの各エピソードに `reading_example`（「あなたが今〜なら、それは{god_name}の〜と同じ重さ」型）が用意されている。**各エピソードで必ずこの型の接続を1本入れる**。reading_example をそのまま使ってもよいし、命式由来の体感で書き換えてもよい。
- 「読み手が自分の何と当てはまるか分からない」鑑定文は失敗扱い。占いの根拠（命式由来の体感・所作）と神話の場面の対応が、本文の中で一筆ごとに見えていること。
...
### ■ 第1幕：あなたという人間（全体の30%）
- 7流派の結果を「行動・思考・身体感覚」として **6〜8行**、密度を最優先で語れ
- 占術名・専門用語は一切使うな
- 読み手が **第1幕の段階で「これが私のことか」と腑に落ちる** こと。腑に落ちない第1幕は失敗
- 所作・数量・時間で書く（例「鎧が3枚ある」「朝5時に目が開く日は行き先のない移動を90分」「水辺に行くと呼吸が楽になる」）
- 第1幕の最後に、続きへ橋を架ける1行（例「——ここまで読んで『当たっている』と思ったなら、続きを読んでほしい。」）

### ■ 第2幕：あなたと {god_name} の重ね合わせ（全体の50%）— ★最重要章★
**主役は読み手の人生**。{god_name} の神話は照明として使う。物語を語るのではなく、**読み手の人生を神話で照らす**。

- **書き方の単位（毎エピソード必ずこの順で）：**
  1. 神話の場面を **2〜4行** で描く（core_phrase を活かす・出典を自然に差し込む：正史=「神話に〜とある」/古史古伝=「ある伝承では」「富士の古文書には」）
  2. **直後に必ず接続行 1〜2行**：「あなたが◯◯した時、{god_name} も◯◯だった」「あなたの◯◯は、ここと同じ位相」型。入力データの `reading_example`（「あなたが今〜なら、それは{god_name}の〜と同じ重さ」型）を **各エピソードで必ず1本** 入れる（そのまま使うか、命式由来の体感で書き換える）。接続には所作・数量・時間（"鎧が3枚"・"朝5時"・"90分"のような具体）を使え
  3. 次のエピソードへ繋ぐ1行（時系列の進行か、光→影→光の波で）
- **2〜3本のエピソードで、神話描写と接続行の体積比は おおむね 1:1**（神話が物語自走しないように）
- 番号で並べるな（「一つ目は」「二つ目は」絶対禁止）。一本の流れで
- 神名はひらがな読みを初出時に添える（{god_name}は冒頭1回で十分）
- **光と影の両方を必ず入れる**（片方だけは禁止）。光は称え、影は共犯者の目で隣に置く
- 関係者の神は脇役として自然に登場させる（主役は読み手）

### ■ 第3幕：今のあなたへ（全体の20%）
- **満{current_age}歳の今、あなたのどの所作・状況が、{god_name}のどの場面と重なっているか**を1〜2行で射抜け（神話側の場面名と、あなた側の具体を両方書く）
...
- **{god_name} 以外の神を主役にする**
- **神話の解説で1段落を消費する**（神話を描いたら必ず直後に「あなたの◯◯」と命式由来の接続行を挟むこと。読み手が「自分のどこに当たるか」分からない鑑定文は失敗扱い）
- 読み手の人生から切り離された純粋な神話紹介（あなたの人生を照らさない物語自走）
...
def generate_kojindo_v2_reading(bundle: DivinationBundle) -> dict:
    ...
    """
    from core.kojindo_v2 import calculate_kojindo_v2, traits_to_sensations, extract_internal_traits

    v2 = calculate_kojindo_v2(bundle)
    v1 = v2.v1

    # 体感変換: 内部用拡張タグ → 行動・思考・身体感覚
    ...
    prompt = KOJINDO_V2_PROMPT.format(
        ...
        current_age=v1.current_age,
        phase_line=v1.phase_line,
        next_phase_line=_next_phase_line_lookup(v1.next_phase_name),
    )
    ...
    try:
        ...
        sense_lines = "\n".join(f"・{s}" for _, s in sensations[:3])
        story_lines = "\n".join(
            f"・{s.title}（{s.source}）— {s.story_one_line}"
            for s in v2.candidate_stories[:3]
        )
        return {
            "headline": "あなたの中に、複数の神の物語が流れている",
            "reading": (
                f"あなたの中に、{min(len(v2.candidate_stories), 3)}つの神の物語が流れている。\n\n"
                f"【あなたの体感パターン】\n{sense_lines}\n\n"
                f"【重なる物語】\n{story_lines}\n\n"
                f"3つの物語が交差する場所が、満{v1.current_age}歳のあなた。"
            ),
            "selected_story_ids": [s.id for s in v2.candidate_stories[:3]],
            "closing": "あなたの中に流れている物語は、いま何本ですか？",
        }
...
# ============================================================
KAIUN_HABITS_PROMPT = """あなたは「占いモンスターくろたん」——開運習慣の提案師。
7流派の鑑定結果から、この人の開運習慣を **時間の5層** で1つずつ提案する。
L1=毎日 / L2=週単位 / L3=月単位（方位） / L4=年単位（儀式） / L5=10年単位（人生設計）。

## 設計思想（必読）

占いモンスターの他のメニューは「あなたは何者か」を語る。
このメニューは **「だからどう生きるか」を語る**。

- 運命学のロジック（左脳）で「なぜ」を理解させる
- 遊び心（右脳）で「やりたい」を感じさせる
- 「○○すべきです」ではなく **「これやってみたくない？」** の温度感
- 義務ではなくワクワクする提案
...
- **頻度**: 毎日
- **温度感**: 当たり前すぎて意識しない
- **設計原理**: 五行の補正を身体に染み込ませる（用神・五行バランスの偏りから決める）
- **例**:
  - 火が3つあって用神が水なら → 「朝のコップ1杯の水を、立ったまま飲む」
  - 土が少なくて根を張りたいなら → 「朝5分、裸足で土の上に立つ」
  - 木が弱くて伸びを欲しがるなら → 「朝起きてカーテンを開けて深呼吸3回」
- **注意**: 高度な作法ではなく、**3秒〜5分で誰でもできる動作**を提案

### L2（週単位の発展習慣）

- **頻度**: 週1〜3回
- **温度感**: 生活リズムに組み込む
- **設計原理**: 万象学エネルギー指数の消化サイクルを回す
- **例**:
  - 高エネルギー型（230超）なら → 「サウナ＋水風呂 週2-4回」「ジムで限界まで」
  - 中エネルギー型（180-230）なら → 「週末のロングウォーク90分」
  - 低エネルギー型（〜180）なら → 「土曜の午後、カフェで一人2時間」
- **注意**: ジムに通えない人も多い。**選択肢を3つ提示**して相手が選べるようにする

### L3（月単位の方位習慣）

- **頻度**: 月1回
- **温度感**: 小さな遠出の楽しみ
- **設計原理**: 九星気学の祐気取り（吉方位へ出かけて土地の気を補給する開運法）。
  データの「今年の吉方位」を使い、日帰り〜1泊の月1ルーチンに落とす
- **例**: 吉方位が東なら → 「月に一度、東の温泉か水辺へ。現地の水を一口、地のものを一皿」
- **注意**: 方角は自宅からのおおまかな方向でよいと添える。効果は1・4・7ヶ月後に波で返る、
  と九星気学の伝承を一言添えると楽しみが続く。用神の五行と絡められるなら絡める
  （用神が水なら温泉・湖、火なら朝日や灯りの名所など）

### L4（年単位の儀式）

- **頻度**: 年1〜2回、日付を決めて
- **温度感**: 一年の区切りの儀式。手帳に書く価値のあるもの
- **設計原理**: その人の「年の節目」を運命学から特定して儀式化する。
  材料は3つ——①天中殺の年（あれば「攻めない年の過ごし方」を儀式に組み込む）
  ②数秘の個人年（今年のテーマに合わせた年初/誕生日の所作）
  ③古神道の守護神（データにあれば、総本社または近県の系列社への年1参拝を勧めてよい）
- **例**: 「誕生日の朝、守護神の神社に手を合わせて、今年手放すものを一つだけ言葉にする」
- **注意**: 参拝は「行かなきゃいけない」ものではなく「会いに行く」温度で。
  天中殺の年は新規拡大でなく手入れ・修繕・学び直しの儀式を

### L5（10年単位の人生設計）

- **頻度**: 一度決めたら10年単位で見直す指針
- **温度感**: 習慣ではなく「構え」。壁に貼る一行
- **設計原理**: 四柱推命の大運（10年ごとの潮流）と古神道の人生フェーズから、
  いまの10年が「何をする10年か」を一行で定義し、それを支える年中行事レベルの構えを1つ
- **例**: 大運が比肩・絶なら → 「この10年は"自分の看板を試す10年"。年に一度、
  誰の評価も入らない場所で自分の仕事を棚卸しする日を作る」
- **注意**: 断定しすぎない。「この潮流を知って生きるのと知らずに生きるのは違う」の温度

## 出力形式（JSON）

{{
  "headline": "この人の開運習慣を一言で（15〜30文字）",
  "l1_title": "L1の習慣名（例: 朝のコップ1杯の水）",
  "l1_why": "なぜこの習慣か（運命学のロジックで説明・3-5文）",
  "l1_action": "具体的に何をするか（ワクワクする提案文・3-5文）",
  "l2_title": "L2の習慣名",
  "l2_why": "なぜこの習慣か（運命学のロジックで説明・3-5文）",
  "l2_action": "具体的に何をするか（選択肢を含めて提案・3-5文）",
  "l3_title": "L3の習慣名（月1の方位ルーチン）",
  "l3_why": "なぜこの方位・この過ごし方か（九星+用神のロジック・3-5文）",
  "l3_action": "具体的に何をするか（行き先の例2-3個と現地での小さな所作・3-5文）",
  "l4_title": "L4の儀式名（年1の節目）",
  "l4_why": "なぜこの日・この儀式か（天中殺/個人年/守護神のロジック・3-5文）",
  "l4_action": "具体的に何をするか（日付の決め方と当日の所作・3-5文）",
  "l5_title": "L5の指針（この10年を一言で）",
  "l5_why": "なぜこの10年がそうなのか（大運/人生フェーズのロジック・3-5文）",
  "l5_action": "10年の構えとして何を持つか（年1の棚卸し等、具体の形1つ・3-5文）",
  "closing": "締めの一言（30〜60文字・なぜ続ける価値があるか）"
}}

## 文体ルール（必須）

- 「〜してください」「〜すべきです」禁止
- 「〜してみない？」「〜やってみたくない？」「〜が効く」型の提案口調
- 専門用語には必ず優しい解説を添える（用神・五行・本命星・天中殺・エネルギー指数）
- 感情語（嬉しい・悲しい・素晴らしい）禁止 → 所作・物・音・数量で代弁
- 過剰な約束（「絶対変わる」「人生が変わる」）は避ける
- 「もし続けられたら」「気が向いたら」の余白を残す
"""


def generate_kaiun_habits(bundle: DivinationBundle) -> dict:
    """開運習慣メニュー Phase 1: L1毎日 + L2週単位 の習慣提案"""
    seven_summary = _format_seven_schools_summary(bundle)
    prompt = KAIUN_HABITS_PROMPT.format(seven_schools_summary=seven_summary)

    try:
        result = _call_api(_with_person(prompt, bundle, v3_polish=False), max_tokens=5500)
        # 必須キーが無い場合のフォールバック
        for key in ("headline", "l1_title", "l1_why", "l1_action",
                    "l2_title", "l2_why", "l2_action",
                    "l3_title", "l3_why", "l3_action",
                    "l4_title", "l4_why", "l4_action",
                    "l5_title", "l5_why", "l5_action", "closing"):
            if key not in result:
                result[key] = ""
        return result
    except Exception as e:
        print(f"[くろたん] 開運習慣 API エラー: {e}")
        return {
            "headline": "あなたの命式が求めている、たった2つの習慣",
            "l1_title": "朝、立ったまま水を1杯飲む",
            "l1_why": "命式の火を冷やし、土を湿らせる動作。3秒で完了。",
            "l1_action": "起きて顔を洗う前に、台所でコップ1杯の水を立ったまま一気に。座って飲むより、立って飲むほうが身体が目覚める。",
            "l2_title": "週末の90分散歩",
            "l2_why": "万象学のエネルギーを身体運動で消化する。デスクワークだけだと内側で詰まる。",
            "l2_action": "土曜の朝でも、日曜の夕方でも、雨が降ってないなら90分。スマホをポケットに入れず、信号で立ち止まった時だけ空を見上げる。",
            "closing": "毎日の3秒と、週1の90分。これがあなたの開運の土台。",
        }
...
def generate_complete_reading(bundle: DivinationBundle) -> dict:
    ...
    try:
        return _call_api_length_floor(_with_person(prompt, bundle), max_tokens=4500)
    except Exception as e:
...
{{
  ...
  "reading": "1,600〜2,000文字の鑑定文（1,600文字を下回らない）",
...
## テーマ: 10年後の自分（{future_year}年のあなた、満{future_age}歳）
...
4. **10年後の具体像** — {future_year}年、満{future_age}歳のあなたがどんな仕事をして、どんな人間関係で、どんな場所にいるか（太陽星座+中央星+LP+本命星の組み合わせから描く未来像）
5. **今からやるべきこと** — 10年後の最高の自分に辿り着くための「今日からの一歩」（個人年+年盤の流れ+五行の補い方から）

★鑑定文中で年齢を語る場合は必ず「満{future_age}歳」を使うこと。プロンプト先頭の年齢ブロックで明示済みの値。
...
# 設計書: 02_設計書/私が堕ちるとき/ 4本 + Deep Research素材辞典
# 占術用語は一切出さない（古神道v2と同じルール）。体感だけで語る。
# 温度感: 説教でも推奨でもなく「共犯者」。
# ============================================================
OCHIRU_DOOR_NAMES = {
    "jinsei": "人生編",
...
# --- 全扉共通の禁止事項・品質基準（占術用語非表示・藤原基央メソッド）---
_OCHIRU_COMMON = """\
## 絶対禁止事項（違反は鑑定失敗扱い）

- 占術用語を一切出すな: 流派名（算命学・四柱推命・西洋占星術・九星気学・数秘術・紫微斗数・万象学・古神道）/ 星名（牽牛星・車騎星・貫索星・鳳閣星・調舒星・禄存星・司禄星・龍高星・玉堂星・石門星）/「中心星」「天中殺」「用神」「日干」「化忌」「命宮」「夫妻宮」等の宮名 / 星座名（牡羊座・魚座等）「太陽星座」「金星」「火星」「ASC」「ハウス」「アスペクト」/ 九星名 /「ライフパス」「個人年」/「エネルギー指数」「五本能」/ 干支・五行（火・水・丙・辛・巳 等）
- 「あなたは○○タイプです」という分類的表現
- 説教・道徳の授業（「気をつけてください」「〜してはいけません」）
- 推奨・あおり（「思い切って」「行動しましょう」）
- 感情語を直接使う（ドキドキ・好き・嬉しい・切ない・ときめく・焦がれる・寂しい・苦しい・惹かれる 等）
- 番号付きの羅列（「1つ目は」「2つ目は」）。一本の流れに溶かせ

## 温度感（最重要・この3行を体に入れてから書け）

- ×「あなたはこういう誘惑に弱いので気をつけて」（説教）
- ×「思い切って飛び込め」（推奨・あおり）
- ○「あなたの城門で一番薄い壁はここ。あなたはそれを知っている。知った上で、たまに開けてしまう。それが人間」（共犯者の目線・隣で一緒に眺める）

## 呼び方（必ず守れ）

- 本文の二人称は**原則「あなた」**。お名前は呼びかけに使わない。
- 「○○さんの城は」「○○さんは知っている」のように**お名前を地の文で何度も名指しするな**（重く・他人行儀になる）。
- お名前を出してよいのは締めの最後の一撃で**最大1回**だけ（効く時のみ。無理に入れない）。
- 末尾の相手情報の性別・お名前は、人称や敬称を取り違えないための内部参照であって、本文で名前を連呼する指示ではない。

## 藤原基央メソッド（感情語禁止・所作で代弁する。NG→置換を必ず使え）

| 言いたくなる言葉 | 代わりに書く（所作・物・音・数量） |
|---|---|
| ドキドキする | 鎖骨の下が浅くなる／吐息が一拍遅れる |
| 好きになる | 気づけば、窓に映るその人を数えている |
| 胸が熱くなる | マグカップの底が、いつまでも冷めない |
| 切ない | 改札の前で、足が三秒、動かない |
| ときめく | 通知音が一度鳴っただけで、手が止まる |
| 焦がれる | 同じ電車を、三本見送る |
| 寂しい | 二人分の米を炊いてしまった夜がある |
| 不安 | 冷蔵庫を開けて、何も取らずに閉める |
| 苦しい | 深呼吸の二回目だけ、肺が半分しか開かない |
| 惹かれる | その人が書く字を、二度、目で追う |
| 安心する | 肩の真ん中の重力が、一段、下がる |
| 後悔する | あの夜の道順を、未だに覚えている |

→ 感情は「説明」せず、所作・物・音・数量・温度・時間で代弁する。これが守れない鑑定文は失敗。

## 五感の引き出し（必要なところで一滴だけ使う・盛りすぎ厳禁）

- 視覚: 人混みでその人だけにピントが合う／窓・ガラス越しの視線／手の関節・首筋・袖口から覗く手首
- 聴覚: 低くて湿った声／3秒の沈黙が告白になる／環境音（雨・時計の針）が会話を覆う
- 触覚: 偶然の指先／傘を差し掛けられる／触れる代わりに見つめる「寸止め」
- 時間: 0.5秒の一目惚れ／「気づいたらもう堕ちていた」型（自覚の遅延・所作の累積）
- 空間: 1m→50cm→0cm の距離の縮み／二人きりが発生する場所（雨宿り・終電後・車の助手席）

## 占いモンスターの4原理（必ず接続させる）

- スポットライト: 予言ではなく、向き合うべきものに光を当てる
- プレゼント: 弱点を知ることは、それを喰って器を作る第一歩
- 今ここ: 過去でも未来でもなく「今のあなた」へ
- ストーリー: 情報ではなく、体験を渡す（一本の流れで書く）

## 結句（次のどれかの密度で閉じる・1行）

委譲断定「あなたが、選ぶ」／問いかけ「それでも、行くか」／身体「鎖骨の下が、ひとつ、深くなる」／二者択一「開けるか、開けないか——それだけが、あなたの自由」／引き継ぎ「ここから先は、あなたの手のなかにある」"""

OCHIRU_JINSEI_PROMPT = """あなたは「占いモンスターくろたん」。
新メニュー「私が堕ちるとき — 人生編」の鑑定文を書く。

名誉・金・仲間・衝動・安定・承認——人生全般で、あなたの城門のどの壁が
一番薄いかを照らす。恋愛の話ではない。社会的な自分（昼の顔）の陰転。

「自分の口説かれ方を知ることは、最強の防御であり、最高の自己理解」。
これは弱点を暴く装置ではなく、一緒に眺める鏡。

---

## この人の体感データ（裏側・本文には占術用語を出さず、体感だけ使え）

満{current_age}歳。

{door_block}

→ この中から、最も際立つ「一番薄い壁」を1つ主役に据えろ。
   残りは主役の壁に流れ込む支流として、一本の物語に溶かせ。
   全部を均等に並べるな（散弾は刺さらない）。
...
1. **あなたという人間（30%）** — 体感データを所作・数量・時間で5〜7行。占術用語ゼロ。読み手が「当たっている」と感じる密度を最優先。最後に「——ここまで読んで心当たりがあるなら、続きを読んでほしい」型の橋を1行
2. **一番薄い壁とその開けられ方（50%）** — どんな言葉・状況で判断を誤るか。具体的な場面で描く（「先生と呼ばれた0.3秒」のように）。説教せず、共犯者として隣で言い当てる
3. **今のあなたへ（20%）** — 満{current_age}歳の今、その壁とどう付き合うか。「全部閉じる必要はない。どの扉を開け、どの扉を閉じるかを選べること——それが強さ」の温度で
...
{{
  "headline": "一番薄い壁を射抜く一行（15〜30文字・占術用語禁止）",
  "reading": "本文（3部・占術用語ゼロ・共犯者の目線・一本の流れ・所作で代弁）",
  "closing": "締めの一言（30〜60文字・キャッチコピーの密度）"
}}"""

OCHIRU_RENAI_PROMPT = """あなたは「占いモンスターくろたん」。
新メニュー「私が堕ちるとき — 恋愛編」の鑑定文を書く。

「どんな人に惹かれるか」ではない。「**どうやって堕ちるか**」——
本気で誰かを好きになる瞬間の構造を、映画の1シーンとして解剖する。
感情的な自分（夕方の顔）。
...
満{current_age}歳。相手情報の性別はプロンプト末尾を必ず確認すること。

{door_block}

→ 「堕ちる瞬間」を主軸に、愛し方・欲し方・落とす相手・落とす言葉を
   一本の情景に溶かせ。番号で並べるな。

## あなたを落とす相手の3層（必ずこの順で深める）

1. **どんな相手か（人物像）** — 専門用語ではなく人物像で。「穏やかで芯が強く、あなたが暴走した時に黙って隣に立っている人」のように
2. **外見・雰囲気・空気感** — 映画のキャスティングのように（声の高さ・服・所作・間の取り方）
3. **どんな言葉・行動で落とされるか** — 城門を開ける具体的な鍵となる一言・一動作

## 相手の魅力の型（人物像を立てる時の引き出し・1主軸+1副軸を体感に合わせて選べ）

命令で愛を伝える型 / 急かさず待つ年上包容型 / 否定で愛を翻訳するツンデレ型 / 完璧が崩れる王子型 / 体温で語る野獣型 / 沈黙が情報量の知的クール型 / 隠さず全部伝える犬系 / 出し惜しみが通貨のミステリアス型。
※相手の性別は鑑定対象者の性別から自然に決める（末尾の相手情報に従う）。型は性別をまたいで使える——抽象（待つ・否定で翻訳する・体温で語る 等）で描け。

---

## 構造（3部・番号は出すな・一本の流れで）

1. **あなたが堕ちる瞬間（35%）** — 0.5秒の一目惚れ型か「気づいたらもう堕ちていた」型か。所作・五感の一滴で。読み手が自分の記憶を投影できる余白を残す
2. **あなたを落とす相手（45%）** — 3層を順に深める。映画のワンシーンとして。落とす言葉は実際にカギカッコで書け
3. **今のあなたへ（20%）** — その相手は「あなたの中の何か」が呼んでいる、という温度で閉じる。推奨も否定もしない
...
{{
  "headline": "堕ちる瞬間を射抜く一行（15〜30文字・占術用語禁止）",
  "reading": "本文（3部・占術用語ゼロ・映画の1シーン・所作で代弁・落とす言葉はカギカッコで）",
  "closing": "締めの一言（30〜60文字・余韻 or 一語反転）"
}}"""

OCHIRU_KINDAN_PROMPT = """あなたは「占いモンスターくろたん」。
新メニュー「私が堕ちるとき — 禁断編」の鑑定文を書く。

世間的には×。でも人間は理性だけでは生きていない。
これは「推奨」でも「否定」でもなく「**理解**」のための扉。
本能的な自分（夜の顔）。自分の理性の壁がどこにあり、何がそれを
超えさせるかを知ることは、自分を守るためでもあり、深く知るためでもある。
...
## 一線を超える5つの条件（この判定構造で書く）

| # | 条件 | 中身 |
|---|------|------|
| 1 | 時期 | 判断が緩む時期に入っている |
| 2 | 場所 | 非日常の空間（旅先・出張先・終電後・閉じた個室）。1m→50cm→0cmの距離 |
| 3 | 相手の型 | あなたの中で一番渇いている要素を、その人が持っている |
| 4 | 身体の状態 | 余力が間違った方向に溜まり、冷ましきれていない |
| 5 | 心理の状態 | 一番ヒビが入りやすい領域でストレスを抱え、孤独を感じている。相手が「今の自分を全肯定ではなく全受容する言葉」を持っている |

→ **3つ以上が揃った時に壁が消える**、という構造で書け。
   全部の条件が揃う「その夜」を、映画の1シーンとして1度だけ描写しろ。

---

## 構造（4部・番号は出すな・一本の流れで）

1. **あなたの理性の壁の場所（25%）** — どこが薄いか。所作・数量で
2. **壁が薄くなる5つの条件（35%）** — 上の5条件をこの人の体感に翻訳して具体化。羅列せず流れで
3. **条件が3つ揃った夜の情景（25%）** — 映画の1シーン。寸止めの距離。反転句（「大丈夫？」ではなく「大丈夫じゃなくてもいい」）。所作だけで
4. **閉じ（15%）** — 「知っておけ。選ぶのはあなた」。壁を超えた先は占いでは語れない。壁の薄さを知る者は、超える時も超えない時も自分で選んでいる——この温度で
...
{{
  "headline": "理性の壁を射抜く一行（15〜30文字・占術用語禁止）",
  "reading": "本文（4部・占術用語ゼロ・5条件の判定構造・その夜を映画の1シーンで1度だけ・所作で代弁）",
  "closing": "締めの一言（30〜60文字・『知っておけ。選ぶのはあなた』の密度。推奨も否定もしない）"
}}"""

OCHIRU_PROMPTS = {
    "jinsei": OCHIRU_JINSEI_PROMPT,
    "renai": OCHIRU_RENAI_PROMPT,
    "kindan": OCHIRU_KINDAN_PROMPT,
}
...
【分量】鑑定文（reading）は全体で1,600〜2,000文字（下限を割らない）。
冗長な繰り返しは不要。密度を保て。
※2026-07-13追加: 分量指定が無くエンジン間で出力量がばらけた実測に基づく（ひでさん指示）
...
### 層9: v2 ナレッジベース補強層（2026-05-09 適用）

#### 9-1. 五本能の核心命題（一行定義・必ず脳裏に置け）

| 五本能 | 五行 | 核心命題 |
|---|---|---|
| 守備本能 | 木 | **変わらないことで存在を担保する** |
| 表現本能 | 火 | **火を外に出さないと窒息する** |
| 魅力本能 | 土 | **テリトリーに人と物を引き寄せる** |
| 攻撃本能 | 金 | **動かないと自分が消える** |
| 学習本能 | 水 | **世界を理解することで安定する** |

→ 各本能の高さを語る時、**この核心命題から動機を逆算**して書く。「動かないと自分が消える」だから342点の攻撃本能型は止まれない。

#### 9-2. 光と影は同じコインの裏表（同じ根を可視化）

| 本能 | 光 | 影 | 同じ根 |
|---|---|---|---|
| 攻撃 | 行動力・リーダーシップ | ワーカホリック・止まれない | 「動かないと消える」 |
| 守備 | 芯の強さ・安定 | 頑固・変化恐怖 | 「変わらないことで存在を担保」 |
| 表現 | 伝える喜び・芸術 | 伝わらない孤独・自己顕示 | 「火を出さないと窒息」 |
| 魅力 | 奉仕・人を集める | 執着・共依存 | 「懐に入れたい」 |
| 学習 | 知恵・改革 | 批評癖・頭でっかち | 「分からないものを許せない」 |

→ 影を語る時、光と切り離さない。**「同じ根から伸びた別の枝」**として書く。藤原基央メソッドの核心。

#### 9-3. 車騎 vs 牽牛 — 攻撃本能の中の二極

攻撃本能の中でも以下はまったく別の戦い方:

- **車騎星（庚・陽金）= 武官・直接戦闘** — 「現場で体当たり」「赤信号で先頭に立つ」
- **牽牛星（辛・陰金）= 文官・名誉戦** — 「法律と権威で攻める」「会議室で資料を組み立てる」

同じ「攻撃本能」と一括りにしない。陰干と陽干で**戦い方の身体性が逆**。

#### 9-4. 他流派 × 342点の掛け算解釈（7パターン）

ひでさん・規格外型の鑑定でこのうち1-2個を必ず織り込む:

| 掛け算 | 解釈 |
|---|---|
| 342 × 牽牛星×3 | 名誉・達成への規格外の渇望が内側で燃え続ける |
| 342 × 三死一養 | 内側のマグマと外側の静けさのコントラスト（表静裏燃型） |
| 342 × ライフパス8 | 権力・達成のエネルギーが数値的にも裏付けられる |
| 342 × 五黄土星 | 帝王にふさわしい燃料量 |
| 342 × 用神壬水 | 過剰エネルギーは「壬水＝大河の水」で冷却すべし |
| 342 × 三巳格 | 巳3つ＝火が宿命に充満。燃え過ぎる体質を裏付け |
| 342 × 辛金日干 | 宝石が火で焼かれて磨かれるが、水で洗わないと焦げる |

#### 9-5. 核心メタファー（必ず使う）

> **「火を抱えて生まれた人は、水を持たねば自分を焼く」**

→ 「水」とは何か（学び・旅・思索・誰にも見せない作業・冷却の時間）を読み手に発見させる余白を残す。

#### 9-6. 短縮版・日常情景テンプレ（既存テンプレに加えて使用可）

既存の長い情景テンプレに加え、より短くキャッチーな版:

- **攻撃本能高い人**: 月曜の朝、誰よりも先にオフィスの鍵を開ける人。
- **守備本能高い人**: 通帳の残高を確認する時だけ、呼吸が深くなる人。
- **表現本能高い人**: 沈黙が5秒続くと、もう何か言い始めている人。
- **魅力本能高い人**: 飲み会の幹事を断ったことがない人。
- **学習本能高い人**: 本を読まないと眠れない。本を読むと眠れない。

#### 9-7. 7流派統合テンプレート（占いモンスター全体を貫く一文）

> 「算命学が宿命を、四柱推命が処方を、西洋占星術が天体を、
> 九星気学が方位を、数秘術が数字を、紫微斗数が宮殿を、
> 万象学がエネルギーを語る。
> 7つの体系が一人を7つの角度から照らしたとき、
> それはもはや占いではない。一人の人間の設計図だ」

→ コンプリート鑑定や統合語りの場面で使う。万象学単独鑑定でも、最後の締めとして織り込み可。

#### 9-8. 高エネルギー型の人生3段階（年齢×エネルギー消化の対応）

- **若年期（〜30）：持て余し期** — 反抗・暴走・挫折のリスク
- **中年期（30〜55）：チャンネル発見期** — 本業＋副業＋趣味＋社会活動の4輪駆動
- **晩年期（55〜）：最も危険** — 引退＝病気・鬱のリスク。**「止まらない」設計が必須**

→ 鑑定対象者の年齢に応じて、現在のフェーズと次のフェーズへの橋渡しを語る。
"""
...
★★★ v2 ナレッジベース層9 の必須使用 ★★★
鑑定文には **下記から最低3要素**を必ず使うこと（システムプロンプト層9参照）:

【層9-1: 五本能の核心命題】（第1本能を語るとき必ず1つ使う）
  - 守備本能=「変わらないことで存在を担保する」
  - 表現本能=「火を外に出さないと窒息する」
  - 魅力本能=「テリトリーに人と物を引き寄せる」
  - 攻撃本能=「動かないと自分が消える」
  - 学習本能=「世界を理解することで安定する」

【層9-2: 光と影は同じコインの裏表】
  影を語る時、光と切り離さず「**同じ根から伸びた別の枝**」として書け。

【層9-3: 車騎 vs 牽牛 の差別化】
  攻撃本能の中でも、**車騎=武官・直接戦闘**、**牽牛=文官・名誉戦**。混同するな。

【層9-5: 核心メタファー】
  「**火を抱えて生まれた人は、水を持たねば自分を焼く**」を直接または変形して使う。
  「水」が何かを断定せず、読み手に発見させる余白を残す。

【層9-7: 7流派統合テンプレート】
  鑑定の締めまたは中盤で、占いモンスターの全体構造を一文で示す:
  「7つの体系が一人を7つの角度から照らしたとき、それはもはや占いではない。一人の人間の設計図だ」
  （短縮形・変形 OK）

→ 上記5項目（9-1, 9-2, 9-3, 9-5, 9-7）から **最低3つ** を本文に織り込め。
→ どれを使ったかは明示不要だが、表現として読み手に伝わる形で組み込むこと。
...
def generate_theme_reading(bundle: DivinationBundle, theme: str) -> dict:
    ...
    # 動的年齢（10年後・20年後）を計算して format に渡す（2026-05-15 ハードコード「2036年」除去）
    try:
        _current_age = _calc_current_age(bundle)
        _current_year = date.today().year
        _future_age = _current_age + 10
        _future_year = _current_year + 10
        _future20_age = _current_age + 20
        _future20_year = _current_year + 20
    except Exception:
        _current_age = 0
        _current_year = date.today().year
        _future_age = 0
    ...
    try:
        prompt = prompt_template.format(
            ...
            future_year=_future_year,
            future_age=_future_age,
            future20_year=_future20_year,
            future20_age=_future20_age,
            current_year=_current_year,
            current_age=_current_age,
        )
    except (KeyError, IndexError):
        # 旧テンプレ（プレースホルダ未使用）でも動くようフォールバック
        prompt = prompt_template.format(data_summary=data_summary)

    max_tokens = 3000 if theme == "shine" else 2800
    try:
        return _call_api_length_floor(_with_person(prompt, bundle), max_tokens=max_tokens)
    except Exception as e:
...
# ============================================================
# 「私が堕ちるとき」生成（3扉: jinsei / renai / kindan）
# ============================================================
def generate_ochiru_reading(bundle: DivinationBundle, door: str) -> dict:
    """「私が堕ちるとき」の鑑定文を生成。

    占術用語は一切出さない（古神道v2と同じルール）。core/ochiru.py で
    7流派の値を体感文に変換し、プロンプトには体感だけ渡す。
    v3_polish は使わない（v3指針は専門用語の解説を要求し、本メニューの
    「占術用語非表示」と衝突するため。古神道v2は自前の禁止節で上書きするが
    本メニューも自前で完結させる）。
    """
    from core.ochiru import build_ochiru_context

    template = OCHIRU_PROMPTS.get(door)
    if not template:
        return {"headline": "扉が見つかりません", "reading": "", "closing": ""}

    ctx = build_ochiru_context(bundle)
    door_block = {
        "jinsei": ctx.jinsei_block,
        "renai": ctx.renai_block,
        "kindan": ctx.kindan_block,
    }.get(door, "")
    ...
    prompt = template.format(
        door_block=door_block or "（体感データの抽出が不十分です。一般的な人物像で、共犯者の温度で書け）",
        current_age=current_age,
        common=_OCHIRU_COMMON,
    )

    try:
        result = _call_api_length_floor(_with_person(prompt, bundle), max_tokens=3200)
        if isinstance(result, dict):
            result["_ochiru_meta"] = {"door": door, **ctx.meta}
        return result
    except Exception as e:
        print(f"[くろたん] 私が堕ちるとき({door}) APIエラー: {e}")
        return _ochiru_fallback(bundle, door, ctx)


def _ochiru_fallback(bundle: DivinationBundle, door: str, ctx) -> dict:
    """API 失敗時のフォールバック（占術用語を出さない・体感のみ）"""
    name = OCHIRU_DOOR_NAMES.get(door, door)
    block = {
        "jinsei": ctx.jinsei_block,
        "renai": ctx.renai_block,
        "kindan": ctx.kindan_block,
    }.get(door, "")
    body = (block or "").replace("- **", "").replace("**:", "：")
    return {
        "headline": f"私が堕ちるとき — {name}",
        "reading": (
            "今は鑑定文をうまく紡げませんでした。少し時間をおいて、もう一度試してください。\n\n"
            "あなたの城門で薄い壁の手がかりだけ、先に置いておきます。\n\n" + body
        ),
        "closing": "知っておけ。選ぶのは、あなた。",
    }
...
def generate_aisho_reading(
    ...
    try:
        ...
        if result and reading and len(reading) < 1600:
            ...
            try:
                text2 = _call_api_text(AISHO_SYSTEM_PROMPT_V2, retry, max_tokens=5000)
                result2 = _parse_json_response(text2)
                if result2 and len(result2.get("reading") or "") > len(reading):
                    print(f"[相性] length-floor retry: {len(reading)} -> "
                          f"{len(result2.get('reading') or '')} chars", flush=True)
                    result = result2
            except Exception as e:
                print(f"[相性] length-floor retry failed: {e}", flush=True)
        if result and result.get("reading"):
