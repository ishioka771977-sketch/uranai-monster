...
{{
  ...
  "reading": "本文の鑑定文。以下の構造で1,800〜2,400文字。",
  "closing": "締めの一言（30〜60文字）"
}}
...
def _with_person(prompt: str, bundle: "DivinationBundle") -> str:
...
def generate_kyusei_reading(bundle: DivinationBundle) -> dict:
    ...
    try:
        return _call_api(_with_person(prompt, bundle), max_tokens=2000)
    except Exception as e:
...
{{"headline": "一言キャッチコピー（15字以内）", "reading": "本文（800〜1200字、改行を適切に入れる）", "closing": "締めの一言（30字以内）"}}"""
    ...
    try:
        return _call_api(_with_person(prompt, bundle), max_tokens=2500)
    except Exception as e:
...
def generate_bansho_reading(bundle: DivinationBundle) -> dict:
    ...
    try:
        ...
        response = client.models.generate_content(
            model="gemini-2.5-pro",
            contents=_with_person(prompt, bundle),
            config=types.GenerateContentConfig(
