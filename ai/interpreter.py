...
{{
  ...
  "reading": "本文の鑑定文（v3削減版）。**800〜1,200文字に厳格に絞る**。冗長な説明・タイプ分類は全削除。密度を上げて文字数を減らす。",
...
{{
  ...
  "reading": "鑑定文（v3削減版）。**800〜1,200文字に厳格に絞る**。冗長な説明・タイプ分類は全削除し、密度を上げる。",
  "closing": "締めの一言（30〜60文字）"
}}
...
{{
  ...
  "reading": "本文の鑑定文（v3削減版）。以下の構造で**800〜1,200文字に厳格に絞る**。冗長な説明・タイプ分類は全削除し、密度を上げる。",
  "closing": "締めの一言（30〜60文字）"
}}
...
def _v3_polish_directive() -> str:
    """v3 ブラッシュアップ削減指針（全流派プロンプト末尾に append する共通指針）

    くろたん v3 統合設計書 §4「削る/残す指針」+ §5「12項目チェックリスト」
    + 藤原基央メソッドの一語反転式テンプレを統合。
...
## 【★★★ v3 ブラッシュアップ最終指針（必読・最優先）★★★】

### 削る（v3で全削除）
1. 「あなたは○○な人です」型のタイプ分類
2. 「○○星は○○を意味します」型の教科書説明
3. 影パターンの5項目羅列（散弾は刺さらない、1点だけ深く刺せ）
4. 「東の星も西の星も〜」の同型反復（1鑑定に最大1〜2回）
...
### 文字数の上限（v3で大幅削減）
- アラカルト（1流派）: **800〜1200字**（v2の1500-2000字 → 縮小）
- 密度を上げて文字数を減らす。冗長な説明は全削除。
...
F1〜F5（藤原基央メソッド）:
- [ ] 感情語（悲しい・嬉しい・素晴らしい）を直接使っていないか
- [ ] 所作・物・音・数量で代弁できているか
- [ ] 一語反転が1箇所以上あるか
- [ ] 環境（他者・町・時間）の動きで主人公を相対化できているか
- [ ] 固有名詞・理由を省略して読み手の人生記憶に流し込めているか
...
【出力形式（JSON）】
{{"headline": "1行で立ち上がる核心フレーズ（15〜30文字）", "reading": "本文（v3削減版・**800〜1,200字に厳格に絞る**。冗長な説明・タイプ分類は全削除し、密度を上げる。改行を適切に入れる）", "closing": "視覚的・象徴的な締めの一撃（30〜60字）"}}"""
...
【鑑定文の構成（v3削減版・**1,000〜1,500文字に厳格に絞る**）】
※冗長な説明・タイプ分類羅列は全削除。算命学接続フレーズ（東東共鳴）は密度高く入れる。
...
このデータをもとに、**1,000〜1,500文字（v3削減版）**の万象学鑑定文を生成してください。
冗長な説明・タイプ分類は全削除し、算命学接続を密度高く語ること。
...
出力はJSON形式:
{{"headline": "キャッチフレーズ（15〜30文字）", "reading": "鑑定文（v3削減版・1,000〜1,500文字）", "closing": "締めの一言（30〜60文字）"}}
"""
