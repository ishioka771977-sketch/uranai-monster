...
アプリはひでさんの耳元でささやく軍師。
"""
# 注釈をすべて文字列遅延評価にする（PEP 563）。
# import時に注釈を評価して落ちる系（例: Optional/list[dict] が未importや
# 旧Python注釈評価で NameError/TypeError）を根本から無効化する。
# 2026-05-17 本番冷間再起動で表面化した import時 NameError 対策。
...
""", unsafe_allow_html=True)
    ...
    # 「私が堕ちるとき」（最も攻めたメニュー・3つの扉）
    st.markdown("""
<div style="text-align:center; margin:16px 0 4px; color:#E08A8A; font-size:0.78em;">
🔥 城門のどの壁が薄いか——人生の誘惑・真剣な恋・禁断の夜
</div>
""", unsafe_allow_html=True)
    if st.button("🔥 私が堕ちるとき 🔥", key="btn_ochiru", use_container_width=True):
        st.session_state.page = "ochiru_doors"
        st.rerun()
...
# ============================================================
# 「私が堕ちるとき」3パターン（人生編 / 恋愛編 / 禁断編）
# 設計: 02_設計書/私が堕ちるとき/ + 新セッション開始指令書B(2026-05-17)
# 共通ルール: record_divination でDB保存 / 復元ビュー対応 / Web Share API対応
# ============================================================
_OCHIRU_DOORS = {
    "jinsei": ("人生編", "名誉・金・仲間・衝動", "人生全般の陰転パターン。城門のどの壁が薄いか"),
    "renai": ("恋愛編", "心が開く瞬間", "真剣な恋に堕ちるとき。あなたの堕ち方と、落とす相手"),
    "kindan": ("禁断編", "理性の壁が消える瞬間", "一線を越える夜の設計図。知っておけ、選ぶのはあなた"),
}


def render_ochiru_doors_page():
    """「私が堕ちるとき」入り口: 3つの扉を選ぶ"""
    render_back_button(target_page="ura_menu", key="ochiru_doors")
    render_star_deco("🔥")
    st.markdown(
        '<div class="uranai-title" style="font-size:1.4em;">運命学からみた、私が堕ちるとき</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div style="text-align:center; color:#A39E93; font-size:0.88em; '
        'margin:10px 14px 4px; line-height:1.7;">'
        '7つの占術が、あなたの「堕ちるパターン」を丸裸にします。<br>'
        '人生の誘惑、真剣な恋、そして禁断の夜。<br>'
        '自分の城門のどこが薄いかを知ることが、最強の開運です。'
        '</div>',
        unsafe_allow_html=True,
    )
    render_gold_divider()
    st.markdown(
        ...
        'margin-bottom:10px;">↓ 開ける扉を選んでタップ ↓</div>',
        unsafe_allow_html=True,
    )

    for door_key, (name, kw, desc) in _OCHIRU_DOORS.items():
        st.markdown(
            f'<div style="margin:6px 0 2px; padding:6px 10px; '
            f'border-left:3px solid #E08A8A;">'
            f'<span style="color:#E0A0A0; font-size:1.05em; font-weight:bold;">扉・{name}</span>'
            f'<span style="color:#8A8478; font-size:0.82em;">　{kw}</span><br>'
            f'<span style="color:#A39E93; font-size:0.82em;">{desc}</span></div>',
            unsafe_allow_html=True,
        )
        if st.button(f"🔥 {name}を開ける", key=f"btn_ochiru_{door_key}",
                     use_container_width=True):
            _start_ochiru(door_key)

    render_back_to_top_button("ochiru_doors")


def _start_ochiru(door_key: str):
    """扉を選択して鑑定生成へ"""
    st.session_state._ochiru_door = door_key
    st.session_state.page = "ochiru_generating"
    st.rerun()


def render_ochiru_generating_page():
    """選択した扉の鑑定文を生成"""
    from ai.interpreter import generate_ochiru_reading

    bundle = st.session_state.bundle
    door = st.session_state.get("_ochiru_door", "jinsei")
    name = _OCHIRU_DOORS.get(door, ("?",))[0]

    render_star_deco("🔥")
    st.markdown(
        f'<div class="uranai-title" style="font-size:1.4em;">✦ {name}：'
        f'あなたの薄い壁を照らしています…</div>',
        unsafe_allow_html=True,
    )

    with st.status(f"✦ 私が堕ちるとき・{name} を鑑定中…", expanded=True) as status:
        st.write("✧ 7占術を裏で読み、あなたの堕ちる構造を一本の物語に編んでいます…")
        result = generate_ochiru_reading(bundle, door)
        status.update(label=f"✦ {name} 鑑定完了 ✦", state="complete")

    if "ochiru_results" not in st.session_state:
        st.session_state.ochiru_results = {}
    st.session_state.ochiru_results[door] = result
    _record_history(
        f"私が堕ちるとき・{name}",
        ["全占術横断"],
        result_json=result if isinstance(result, dict) else None,
    )
    st.session_state.page = "ochiru_result"
    st.rerun()


def render_ochiru_result_page():
    """「私が堕ちるとき」結果画面（共有・他の扉・戻る）"""
    render_back_button(target_page="ochiru_doors", key="ochiru_result")
    bundle = st.session_state.bundle
    door = st.session_state.get("_ochiru_door", "jinsei")
    name = _OCHIRU_DOORS.get(door, ("?",))[0]
    data = st.session_state.get("ochiru_results", {}).get(door, {})

    d = bundle.person.birth_date
    nm = bundle.person.name
    render_star_deco("🔥")
    title_text = f"{nm}さん — " if nm else ""
    st.markdown(
        f'<div class="uranai-title" style="font-size:1.35em;">'
        f'{title_text}私が堕ちるとき・{name}</div>',
        unsafe_allow_html=True,
    )
    render_gold_divider()
    ...
    if headline:
        st.markdown(
            f'<div style="text-align:center; font-size:1.2em; color:#E0A0A0; '
            f'font-weight:bold; margin:15px 0;">「{headline}」</div>',
            unsafe_allow_html=True,
        )
    if reading:
        st.markdown(
            f'<div class="reading-text" style="line-height:2.0; '
            f'white-space:pre-wrap;">{reading}</div>',
            unsafe_allow_html=True,
        )
    if closing:
        st.markdown(
            f'<div style="text-align:center; color:#D4B96A; font-style:italic; '
            f'margin:20px 0; font-size:1.02em; line-height:1.8;">✦ {closing} ✦</div>',
            unsafe_allow_html=True,
        )

    render_gold_divider()

    # 共有（テキスト / ダイジェスト / PDF）— テーマ結果と同じ仕組み
    _oc_title = f"{nm}さん — 私が堕ちるとき・{name}" if nm else f"私が堕ちるとき・{name}"
    _oc_subtitle = f"{d.year}年{d.month}月{d.day}日生まれ"
    _oc_text = _build_share_text(_oc_title, _oc_subtitle, headline, reading, closing)
    _oc_pdf = _build_pdf_html(_oc_title, _oc_subtitle, headline, reading, closing)
    _oc_digest = _build_share_digest(_oc_title, headline, closing)
    _render_share_buttons(_oc_text, f"ochiru_{door}", _oc_pdf, _oc_digest)
    ...
    st.markdown(
        ...
        '<span style="color:#E0A0A0; font-size:1.0em;">✦ 他の扉も開ける ✦</span></div>',
        unsafe_allow_html=True,
    )
    cols = st.columns(3)
    for i, (dk, (dn, _kw, _ds)) in enumerate(_OCHIRU_DOORS.items()):
        with cols[i]:
            label = f"✓ {dn}" if dk == door else f"🔥 {dn}"
            if st.button(label, key=f"btn_ochiru_other_{dk}",
                         use_container_width=True):
                _start_ochiru(dk)
    ...
    col1, col2 = st.columns(2)
    with col1:
        if st.button("✦ コース鑑定へ", key="btn_ochiru_to_course"):
            st.session_state.page = "ura_menu"
            st.rerun()
    with col2:
        if st.button("✧ もう一度鑑定する ✧", key="btn_ochiru_restart"):
            st.session_state.page = "input"
            st.session_state.bundle = None
            st.session_state.recommendation = None
            st.session_state.course_results = {}
    ...
    render_back_to_top_button("ochiru_result")
...
def _life_curve_svg(curve, taiun_list, current_age):
    """人生運勢カーブのSVGを生成（0〜max歳・大運帯・天中殺年・現在地入り）"""
    W, H = 720, 250
    L, R, T, B = 42, 706, 26, 200
    max_age = curve[-1]["age"]

    def x(age):
        return L + (R - L) * age / max_age

    def y(score):
        return B - (B - T) * score / 10

    parts = []
    # 大運の交互背景帯
    for i, e in enumerate(taiun_list):
        x0, x1 = x(e["start_age"]), x(min(e["end_age"] + 1, max_age))
        if x0 >= R:
            continue
        fill = "rgba(191,163,80,0.07)" if i % 2 == 0 else "rgba(191,163,80,0.02)"
        parts.append(f'<rect x="{x0:.1f}" y="{T}" width="{max(0, x1 - x0):.1f}" height="{B - T}" fill="{fill}"/>')
        if x0 + 24 < R:
            parts.append(f'<text x="{x0 + 4:.1f}" y="{T + 12}" font-size="9" fill="#8A8478">{e["kanshi"]}</text>')
    # 目盛り
    for s in (0, 5, 10):
        parts.append(f'<line x1="{L}" y1="{y(s):.1f}" x2="{R}" y2="{y(s):.1f}" stroke="#2A2A2A" stroke-width="1"/>')
        parts.append(f'<text x="{L - 6}" y="{y(s) + 3:.1f}" font-size="10" fill="#8A8478" text-anchor="end">{s}</text>')
    for a in range(0, max_age + 1, 10):
        parts.append(f'<text x="{x(a):.1f}" y="{B + 16}" font-size="9" fill="#8A8478" text-anchor="middle">{a}歳</text>')
    # 面と折れ線
    pts = " ".join(f"{x(p['age']):.1f},{y(p['score']):.1f}" for p in curve)
    area = f"{L},{B} " + pts + f" {R},{B}"
    parts.append(f'<polygon points="{area}" fill="url(#goldfade)"/>')
    parts.append(f'<polyline points="{pts}" fill="none" stroke="#D4B96A" stroke-width="2" stroke-linejoin="round"/>')
    # 天中殺年の点
    for p in curve:
        if p["is_tcs_year"]:
            parts.append(f'<circle cx="{x(p["age"]):.1f}" cy="{y(p["score"]):.1f}" r="2.4" fill="#C47A6A"/>')
    # 現在地
    if 0 <= current_age <= max_age:
        cx = x(current_age)
        cy = y(curve[current_age]["score"])
        parts.append(f'<line x1="{cx:.1f}" y1="{T}" x2="{cx:.1f}" y2="{B}" stroke="#D4B96A" stroke-width="1" stroke-dasharray="3,3"/>')
        parts.append(f'<circle cx="{cx:.1f}" cy="{cy:.1f}" r="5" fill="#D4B96A" stroke="#0A0A0A" stroke-width="2"/>')
        label_anchor = "end" if cx > R - 60 else "start"
        lx = cx - 8 if label_anchor == "end" else cx + 8
        parts.append(f'<text x="{lx:.1f}" y="{T + 12}" font-size="11" fill="#D4B96A" font-weight="bold" text-anchor="{label_anchor}">今 {current_age}歳</text>')
    svg = (
        f'<svg viewBox="0 0 {W} {H}" xmlns="http://www.w3.org/2000/svg" '
        f'style="width:100%; height:auto; background:#111111; border:1px solid #2A2A2A; border-radius:10px;">'
        f'<defs><linearGradient id="goldfade" x1="0" y1="0" x2="0" y2="1">'
        f'<stop offset="0%" stop-color="rgba(212,185,106,0.30)"/>'
        f'<stop offset="100%" stop-color="rgba(212,185,106,0)"/></linearGradient></defs>'
        + "".join(parts) + "</svg>"
    )
    return svg
...
def _render_kaiyun_taiun_tab(person, person_data):
    from core.kaiyun import (
        calc_taiun, get_current_taiun, DAILY_KANSEI,
        calc_taiun_period_score, calc_life_curve,
    )
    ...
    # --- 人生運勢カーブ（2026-07-18追加）---
    st.markdown(
        '<div style="color:#BFA350; font-weight:bold; margin:10px 0 8px; font-size:1.1em;">📈 人生運勢カーブ（0〜90歳）</div>',
        unsafe_allow_html=True,
    )
    curve = calc_life_curve(person_data, birth_date, gender_str, max_age=90)
    st.markdown(_life_curve_svg(curve, taiun_list, current_age), unsafe_allow_html=True)
    st.markdown(
        '<div style="color:#5A5A5A; font-size:0.72em; margin:4px 0 14px;">'
        '金の線=大運の土台+年運の揺らぎ / <span style="color:#C47A6A;">●</span>=天中殺の年 / 縦帯=大運の切り替わり<br>'
        '※ 高い=良い・低い=悪いではなく、「攻めが実る時期」と「内を耕す時期」のリズムを表します</div>',
        unsafe_allow_html=True,
    )

    st.markdown(
        f'<div style="color:#BFA350; font-weight:bold; margin:10px 0 8px; font-size:1.1em;">'
        f'🔮 大運タイムライン（現在{current_age}歳）— 各期をタップで深掘り</div>',
        unsafe_allow_html=True,
    )

    # タイムライン表示（各期expander・2026-07-18 深掘り化）
    for entry in taiun_list:
        is_current = (entry == current_taiun)
        is_tcs = entry["is_taiun_tenchusatsu"]
        marker = "★ " if is_current else ("⚠ " if is_tcs else "")
        ...
        pscore = calc_taiun_period_score(entry, person_data["day_kan"])
        theme10 = kansei_info["theme"].replace("の日", "の10年")
        header = f'{marker}{entry["kanshi"]}  {age_range}（{year_range}）— {theme10}  スコア{pscore["score"]}'

        with st.expander(header, expanded=is_current):
            tcs_years_html = ""
            if entry["tenchusatsu_years"]:
                _tys = "・".join(str(yy) for yy in entry["tenchusatsu_years"])
                tcs_years_html = (
                    f'<div style="color:#C47A6A; font-size:0.85em; margin:6px 0;">'
                    f'⚠ この期間の天中殺の年: {_tys}——この年は拡大より手入れ・学び直しに</div>'
                )
            chips = ""
            if pscore["reasons"]:
                chip_items = "".join(
                    ...
                    f'border-radius:10px; font-size:0.72em; '
                    f'color:{"#7CA3B8" if pt.startswith("+") else "#C47A6A"};">{pt} {label}</span>'
                    for pt, label in pscore["reasons"]
                )
                chips = (
                    '<div style="margin-top:8px; padding-top:6px; border-top:1px solid rgba(191,163,80,0.15);">'
                    '<span style="color:#8A8478; font-size:0.72em;">スコアの内訳（基準5点）: </span><br>'
                    f'{chip_items}</div>'
                )
            now_badge = ""
            if is_current:
                now_badge = '<div style="color:#D4B96A; font-size:0.8em; margin-bottom:6px;">★ いまここを歩いています</div>'
            st.markdown(f"""
<div style="color:#F0EBE0; font-size:0.88em; line-height:1.8;">
{now_badge}
<div style="display:flex; flex-wrap:wrap; gap:4px 14px; margin-bottom:6px;">
  <span style="color:#8A8478; font-size:0.85em;">五行: <span style="color:#D4B96A;">{entry["gogyo"]}</span></span>
  <span style="color:#8A8478; font-size:0.85em;">本能: <span style="color:#D4B96A;">{entry["honnou"]}</span></span>
  <span style="color:#8A8478; font-size:0.85em;">通変星: <span style="color:#D4B96A;">{entry["kansei"]}</span></span>
</div>
<div style="margin:4px 0;"><span style="color:#D4B96A;">◎ この10年でやると良いこと:</span> {kansei_info["do"]}</div>
<div style="margin:4px 0;"><span style="color:#C47A6A;">△ 注意:</span> {kansei_info["dont"]}</div>
{tcs_years_html}
{chips}
</div>
""", unsafe_allow_html=True)
