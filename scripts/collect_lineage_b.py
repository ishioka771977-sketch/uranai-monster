...
- 検索キーワードは「浅間」のような短い形を優先する(「浅間神社」だと
  「冨士浅間神社」「浅間社」等が完全一致検索で漏れる。新潟で実測)
- 神社を発見してもaction=followで探索を続けてよい(発見分は蓄積される)。
  同じ県内に別の浅間神社がまだありそうなら索引・検索を続ける
- 祭神は詳細ページにしか無いことが多い。一覧で社名に「浅間」を見つけたら
...
def run(prefs: list[str] | None = None) -> None:
    ...
    for i, pref in enumerate(prefs, 1):
        ...
        if not entry.get("url"):
            ...
            t0 = time.time()
            try:
                # search_url(系列語の検索結果URL・実在確認済み)があれば入口として優先
                rec = collect_pref(client, pref, entry.get("search_url") or entry["url"])
...
def export_lineage() -> None:
    """採用候補をアプリ用 data/kojindo/shrines/lineage_v3.json に書き出す
            ...
            for s in rec.get("shrines", []):
                ...
                out.append({
                    ...
                    "sources": [{
                        ...
                        "source_type": "県神社庁DB" if s.get("_method") != "A" else "神社公式等(方式A検証済み)",
                    }],
                })
